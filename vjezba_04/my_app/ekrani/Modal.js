...
const Modal = ({navigation}) => {
  ...
  const handleSubmit = useCallback(() => {
    if (!name) {
      ...
      Alert.alert('Please enter at least 3 colors to add new pallete scheme!');
    }  */ else {
      //navigation.goBack();
      const newPaleteName = {paletteName: name, colors: selectedColors};
      navigation.navigate('Home', newPaleteName);
    }
  }, [name, selectedColors]); //ovo se pozove kada korisnik pritisne botun
    ...
    async (value, color) => {
      if (value === true) {
        ...
        setSelectedColors(colors =>
          colors.filter(
            pickedColors => pickedColors.colorName !== color.colorName,
          ),
        );
        console.log(selectedColors);
      }
          ...
          <View style={styles.color}>
            ...
            <Switch
              ...
              onValueChange={selected => {
                ...
                console.log(item);
                handleColorPicker(selected, item);
              }}
            />
          </View>
        )}
...
const styles = StyleSheet.create({
  input: {borderColor: 'gray', borderWidth: 1, padding: 10, marginBottom: 50},
